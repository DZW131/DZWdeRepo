...
def test_v12_has_exactly_four_projection_parameters_and_no_classifier():
    module = OSMFV12Factorizer(512)
    ...
    assert "semantic_classifier" not in inspect.getsource(OSMFV12Factorizer)


@pytest.mark.parametrize("dtype", [torch.float32, torch.bfloat16])
def test_v12_partition_initialization_is_exact_identity(dtype):
    module = OSMFV12Factorizer(8)
...
def test_v12_cuda_batch20_bf16_exact_identity_under_a0_tf32():
    module = OSMFV12Factorizer(512).cuda()
...
def test_a0_checkpoint_delta_is_only_four_projection_keys():
    ...
    from network.resnet38_cls_osmf_v12 import Net as V12Net

    baseline = A0Net(n_class=4)
    v12 = V12Net(n_class=4)
    incompatible = v12.load_state_dict(baseline.state_dict(), strict=False)
...
def test_v12_optimizer_groups_cover_every_new_parameter_once():
    from network.resnet38_cls_osmf_v12 import Net
...
def test_v12_full_cam_path_is_exact_at_initialization():
    ...
    with torch.no_grad():
        ...
        v12_outputs = v12.forward_cam(image)
        aux = v12.forward_osmf_features(image)
    assert torch.equal(aux["input"], aux["reconstruction"])
    assert all(
        torch.equal(left, right)
        for left, right in zip(baseline_outputs, v12_outputs)
    )


def test_v12_constants_remain_frozen():
    assert OSMF_LAMBDA_SEM == 0.05
    assert OSMF_LAMBDA_MORPH == 0.05
    assert OSMF_LAMBDA_ORTH == 0.05
    assert OSMF_LAMBDA_REC == 0.10
    assert OSMF_EQUIVARIANCE_INTERVAL == 4


def test_v12_module_creation_does_not_advance_global_rng():
    from network.resnet38_cls import Net as A0Net
    from network.resnet38_cls_osmf_v12 import Net as V12Net
