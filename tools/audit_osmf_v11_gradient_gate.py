"""Run frozen OSMF-v1.1 8-batch readiness or gated 128-batch Phase 0."""
...
from tools.osmf_v11_audit.report import (
    build_main_table,
    make_figures,
    write_csv,
    write_report,
)
from train_osmf_v11 import sshr_classification_loss
...
def _load_checkpoint(model: Net, checkpoint: Path):
    ...
    if set(incompatible.missing_keys) != EXPECTED_MISSING_KEYS:
        raise AssertionError(
            f"Unexpected v1.1 missing keys: {sorted(incompatible.missing_keys)}"
        )
...
def _sha256_json_proof(path: Path, expected_decision: str, commit: str) -> str:
    ...
    proof_commit = proof.get("osmf_v11_commit", proof.get("audit_commit"))
...
def _hard_stop_reason(gate, ratio_rows, representation_rows, finite):
    ...
    objectives = ("sem_pres",) if gate == "readiness" else OBJECTIVE_NAMES
...
def _source_hashes():
    names = (
        ...
        "network/resnet38_cls_osmf_v11.py",
        "train_osmf_v11.py",
        "train_sshr.py",
        "tool/GenDataset.py",
        "tool/torchutils.py",
...
def main():
    ...
    parity_sha = _sha256_json_proof(
        args.parity_summary, "OSMF_V11_PARITY_PASS", args.audit_commit
    )
    readiness_sha = None
    if args.gate == "phase0":
    ...
    if start_error >= 1e-6 or not start_loss["finite"]:
        raise RuntimeError(
            f"OSMF_V11_PARITY_NOGO start-state identity/finite failure: {start_error}"
        )
    ...
    summary = {
        ...
        "authorized_batches": authorized_batches,
        "audit_steps_completed": sorted({int(row["step"]) for row in ratio_rows}),
        "audit_commit": args.audit_commit,
        "baseline_commit": BASELINE_COMMIT,
        "checkpoint_sha256": checkpoint_sha,
        "parity_summary_sha256": parity_sha,
        ...
        "next_gate_authorized": (
            "phase0_128b"
            if decision == "OSMF_V11_SEMANTIC_READINESS_PASS"
            else "none"
        ),
