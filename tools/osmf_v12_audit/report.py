"""Deterministic tables, figures, and reports for OSMF-v1.2 audits."""
...
def make_figures(
    ...
    _plot(
        ...
        "Cumulative OSMF-v1.2 parameter movement",
...
def write_report(
    ...
    report_name = (
        "osmf_v12_semantic_readiness_report.md"
        if summary["gate"] == "readiness"
        else "osmf_v12_phase0_report.md"
    )
    report = output_dir / "docs" / report_name
    lines = [
        f"# OSMF-v1.2 {summary['gate'].title()} Audit",
        "",
        "## 1. Decision",
        "",
        f"**{summary['decision']}**",
    ...
    budget = summary["gradient_budget"]
    lines.extend(
        [
            "",
            "## 5. Gradient-budget gate answers",
            "",
            f"1. Semantic gradient budget: max={budget['sem_pres']['max']:.6g}, mean={budget['sem_pres']['mean']:.6g}, p95={budget['sem_pres']['p95']:.6g}.",
            f"2. Morphology gradient budget: max={budget['eq']['max']:.6g}, mean={budget['eq']['mean']:.6g}, p95={budget['eq']['p95']:.6g}.",
            f"3. Both semantic parameters active: {sem['semantic_parameters_active']}; morphology objective active: {mechanism['morphology_eq_gradient_active']}.",
            f"4. Semantic response non-degenerate: {sem['response_non_degenerate']}; end R_Z={sem['response_ratio_end']:.6g}, SemAgree={sem['agreement_end']:.6g}.",
            f"5. End reconstruction cosine: {mechanism['reconstruction_cosine_end']:.8f}.",
            f"6. End S/M RMS ratio: {mechanism['semantic_morphology_rms_ratio_end']:.6g}.",
            f"7. Next gate authorized: {summary['next_gate_authorized']}.",
            "",
