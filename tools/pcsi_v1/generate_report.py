...
def main():
    ...
    section(1,"Executive Decision",f"**{result['FINAL_DECISION']}**。Phase A proved the pre-CCRA causal path; E5 shows C1–C0 {pp(scores['C1']['mIoU']-scores['C0']['mIoU'])} and C2–C1 {pp(scores['C2']['mIoU']-scores['C1']['mIoU'])}. Gate C1 minimum={result['C1_MINIMUM_GO']}, C2 PDSR={result['C2_PDSR_GO']}, strong={result['C2_STRONG_GO']}. No Full25 in this round.")
    ...
    section(28,"True-vs-Rival","Rival exit RER: "+", ".join(f"{v} {pct(resp[v]['RER']['component_rate'])}" for v in ("C1","C2"))+"; correct exit conditional rate: "+", ".join(f"{v} {pct(resp[v]['CorrectRER']['component_rate'])}" for v in ("C1","C2"))+"; Hard-M1 true−rival L5 margin mean: "+", ".join(f"{v} {fmt(resp[v]['true_rival_margin_hard_mean'])}" for v in ("C1","C2"))+".")
    ...
    section(39,"Exact Next Step","Stop at this preregistered Phase0 decision. If strong/qualified GO, design a separately preregistered follow-up testing the responsible mechanism; if NOGO, close this PLIP/PDSR architecture route and revisit the CCRA/HQMR interface or semantic source. Do not extend this run to 10/20/25 epochs or tune gamma against validation GT.")
