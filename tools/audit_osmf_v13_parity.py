"""OSMF-v1.3 Phase -1.1 exact BCSS validation parity recheck."""
...
def write_report(output_dir: Path, summary: dict) -> None:
    ...
    lines = [
        ...
        f"- OSMF-v1.3 commit: `{summary['osmf_v13_commit']}`",
    ...
    (output_dir / "osmf_v13_parity_report.md").write_text(
        "\n".join(lines) + "\n", encoding="utf-8"
    )
...
def parse_args():
    ...
    parser.add_argument("--osmf-v13-commit", required=True)
...
def main():
    ...
    if len(args.osmf_v13_commit) != 40:
        raise ValueError("--osmf-v13-commit must be a full Git SHA")
    ...
    summary = {
        "decision": "OSMF_V13_PARITY_PASS" if parity_pass else "OSMF_V13_PARITY_NOGO",
        "frozen_a0_commit": FROZEN_A0_COMMIT,
        "osmf_v13_commit": args.osmf_v13_commit,
