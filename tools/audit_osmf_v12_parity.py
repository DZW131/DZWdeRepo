"""OSMF-v1.2 Phase -1.1 exact BCSS validation parity recheck."""
...
from network.resnet38_cls_osmf_v12 import Net_CAM as V12NetCAM
...
def load_models(checkpoint: Path):
    ...
    v12 = V12NetCAM(n_class=4)
    incompatible = v12.load_state_dict(state, strict=False)
    missing = set(incompatible.missing_keys)
    unexpected = set(incompatible.unexpected_keys)
    if missing != EXPECTED_MISSING_KEYS:
        raise AssertionError(f"Unexpected v1.2 missing keys: {sorted(missing)}")
    if unexpected:
        raise AssertionError(f"Unexpected checkpoint keys: {sorted(unexpected)}")
    for key, value in state.items():
        if not torch.equal(value.cpu(), v12.state_dict()[key].cpu()):
            raise AssertionError(f"Frozen A0 key changed while loading: {key}")
    baseline = baseline.cuda()
    v12 = v12.cuda()
    baseline.eval()
    v12.eval()
    return baseline, v12, sorted(missing), sorted(unexpected)


def write_report(output_dir: Path, summary: dict) -> None:
    prediction = summary["prediction_parity"]
    lines = [
        "# OSMF-v1.2 Phase -1.1 Parity Recheck",
        "",
        "## Decision",
        "",
        f"**{summary['decision']}**",
        ...
        f"- OSMF-v1.2 commit: `{summary['osmf_v12_commit']}`",
    ...
    lines.extend(
        [
            ...
            f"- A0/v1.2 mIoU: {prediction['a0_mIoU']:.10f} / {prediction['v12_mIoU']:.10f}",
            f"- Absolute mIoU difference: {prediction['mIoU_absolute_difference']:.10g}",
            f"- A0/v1.2 mDice: {prediction['a0_mDice']:.10f} / {prediction['v12_mDice']:.10f}",
    ...
    (output_dir / "osmf_v12_parity_report.md").write_text(
        "\n".join(lines) + "\n", encoding="utf-8"
    )
...
def parse_args():
    ...
    parser.add_argument("--osmf-v12-commit", required=True)
...
def main():
    ...
    if len(args.osmf_v12_commit) != 40:
        raise ValueError("--osmf-v12-commit must be a full Git SHA")
    ...
    set_seed(20260817)
    baseline, v12, missing, unexpected = load_models(args.checkpoint)
    ...
    tensor_results = {
        "random_input": tensor_parity(
            baseline, v12, random_image, torch.bfloat16
        ),
        "real_validation_input": tensor_parity(
            baseline, v12, real_image, torch.bfloat16
        ),
    }
    exact_output_keys = (
        "cam56_max_abs",
    ...
    v12_score, v12_capture = official_inference_with_capture(
        v12, args.val_root, infer_args
    )
    if not np.array_equal(a0_capture["gt"], v12_capture["gt"]):
        raise AssertionError("Validation GT order changed between parity runs")
    differing_pixels = int(
        np.count_nonzero(a0_capture["prediction"] != v12_capture["prediction"])
    )
    a0_miou, v12_miou = float(a0_score["Mean IoU"]), float(v12_score["Mean IoU"])
    a0_mdice, v12_mdice = float(a0_score["Mean Dice"]), float(v12_score["Mean Dice"])
    miou_diff, mdice_diff = abs(a0_miou - v12_miou), abs(a0_mdice - v12_mdice)
    parity_pass = bool(
        tensor_pass
    ...
    v12_parameters = sum(parameter.numel() for parameter in v12.parameters())
    summary = {
        "decision": "OSMF_V12_PARITY_PASS" if parity_pass else "OSMF_V12_PARITY_NOGO",
        "frozen_a0_commit": FROZEN_A0_COMMIT,
        "osmf_v12_commit": args.osmf_v12_commit,
        ...
        "parameters": {
            ...
            "v12_total": v12_parameters,
            "delta": v12_parameters - a0_parameters,
            "overhead_percent": 100.0
            * (v12_parameters - a0_parameters)
            / a0_parameters,
            "new_trainable_tensors": len(tuple(v12.osmf_28_1.parameters())),
        },
        "environment": {
