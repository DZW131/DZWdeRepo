...
def main():
    ...
    for variant in ("P0","P1","P2","P3"):
        block=profile[variant]; full=block.get('full_inference_params',counts['P1']['hqmr_frozen_params']); cost_table.append(f"| {variant} | {block['gflops_per_view']:.2f} | {full} | {block.get('phase0_trainable_params',0)} | {block.get('eval_fps_3view',block.get('estimated_3view_fps',0)):.2f} | {block.get('eval_peak_vram_gib',block.get('peak_vram_gib',0)):.2f} |")
    ...
    add("Bootstrap CI",f"2000 次 paired image bootstrap，seed42。P3−P0 mIoU 95% CI={r['bootstrap']['miou_P3_P0']['ci95']} pp，包含零；P3−P2 95% CI={r['bootstrap']['miou_P3_P2']['ci95']} pp，虽偏负但绝对量级仅约千分之一百分点。全量机器可读统计：`{json.dumps(r['bootstrap'])}`。")
