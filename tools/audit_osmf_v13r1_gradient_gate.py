"""Run gated OSMF-v1.3-R1 8-batch readiness or 128-batch Phase-0S."""
...
from tools.osmf_v13r1_audit.report import write_csv, write_report
...
def main():
    ...
    if args.gate == "phase0s":
        ...
        readiness_sha = _proof(args.readiness_summary, "OSMF_V13R1_READINESS_PASS", "audit_commit", args.audit_commit)
    ...
    for step in range(1, authorized + 1):
        ...
        if step in audit_steps or step in active_steps:
            print("[OSMF-v1.3-R1] " + json.dumps({"gate": args.gate, "step": step, "causal_delta": None if step not in active_steps else causal_rows[-1]["delta"]}, sort_keys=True), flush=True)
        if not finite:
            break
    ...
    if args.gate == "readiness":
        decision, reasons, ratio_stats, causal = readiness_decision(finite=finite, ratio_rows=ratio_rows, representation_rows=representation_rows, parameter_summary=parameter_summary, morphology_graph_expected=morphology_graph_expected, semantic_path_active=semantic_active, causal_rows=causal_rows, sshr_loss_stable=sshr_stable)
        fixed_summary = None
    else:
    ...
    summary = {
        ...
        "morphology_graph_expected": morphology_graph_expected,
