...
def write_rddr_optimizer_audit(path, model, optimizer, mode):
    if not path:
        return
    named = dict(model.named_parameters())
    target_names = [name for name in named if name.startswith('dross_disposal.')]
    membership = {}
    for name in target_names:
        ...
        for group_index, group in enumerate(optimizer.param_groups):
            hits.extend(
                group_index for candidate in group['params'] if candidate is parameter
            )
        membership[name] = hits
    if mode in {'uc', 'dd'}:
        if not target_names or any(len(hits) != 1 for hits in membership.values()):
            raise AssertionError(f"DDA optimizer membership is not exactly once: {membership}")
    payload = {
        'mode': mode,
        'dda_parameter_count': int(sum(named[name].numel() for name in target_names)),
        'dda_parameters': {
            name: {'shape': list(named[name].shape), 'optimizer_groups': hits}
...
def validate_rddr_phase1_training_contract(args):
    mode = getattr(args, 'rddr_phase1_mode', 'none')
    if mode == 'none':
        return
    if args.dataset != 'bcss':
        raise AssertionError('RDDR Phase-1 is frozen to BCSS')
    if args.eval_every != 0:
        raise AssertionError('RDDR Phase-1 forbids train-time test access; use --eval_every 0')
    if args.seed != 42 or args.max_epoches != 25:
        raise AssertionError('RDDR Phase-1 requires seed42 Full25')
...
def train_phase(args):
    ...
    set_seed(args.seed)
    validate_rddr_phase1_training_contract(args)
    rddr_phase1_mode = getattr(args, 'rddr_phase1_mode', 'none')
    model = getattr(importlib.import_module(args.network), 'Net')(
        n_class=args.n_class,
        rddr_phase1_mode=rddr_phase1_mode,
    ).cuda()
    ...
    write_rddr_optimizer_audit(
        getattr(args, 'optimizer_audit_path', None), model, optimizer, rddr_phase1_mode
    )
    ...
    for ep in range(args.max_epoches):
        ...
        write_epoch_curve(getattr(args, 'training_curve_path', None), training_curve)
...
if __name__ == '__main__': 
    ...
    parser.add_argument("--rddr_phase1_mode", "--rddr-phase1-mode", default="none", choices=["none", "uc", "dd"])
    parser.add_argument("--save_epoch_milestones", "--save-epoch-milestones", default="", type=str)
    parser.add_argument("--training_curve_path", "--training-curve-path", default=None, type=str)
    parser.add_argument("--optimizer_audit_path", "--optimizer-audit-path", default=None, type=str)
