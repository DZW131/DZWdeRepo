"""OSMF-v1.2 semantic-preservation factorization.

The factorization/reconstruction graph and objectives are frozen from
OSMF-v1.1. Version 1.2 changes only the semantic and morphology gradient
budgets; semantic specialization remains supervised by the pretrained SSHR
``ic1`` response geometry.
"""
...
# OSMF-v1.2's only scientific change from v1.1 is the conservative budget for
# the two specialization objectives. Architecture and objective definitions are
# intentionally reused without alteration.
OSMF_LAMBDA_SEM = 0.05
OSMF_LAMBDA_MORPH = 0.05
OSMF_LAMBDA_ORTH = 0.05
OSMF_LAMBDA_REC = 0.10


class OSMFV12Factorizer(nn.Module):
