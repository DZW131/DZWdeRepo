"""Frozen CQRF with zero-parameter Mass-Preserving Ownership Mixture Decoding."""
...
from network.momd import mixture_class_bce, mixture_decode, pca_reference_envelope


class MOMDNet(CQRFNet):
    """Use detached CCRA ownership as normalized semantic mixture coefficients."""

    def forward(self, image, labels, step=0, run_pmec=False):
        if labels is None:
            raise ValueError("MOMD Phase-0 requires train-split image labels")
        ...
        for stage_index, (query, confidence, detail, memory_hw) in enumerate(inputs, 1):
            embedding = self.mask_embeddings[stage_index - 1](query)
            base_logits = torch.einsum("bqd,bdhw->bqhw", embedding, pixel_feature)
            stage = {"query": query, "mask_embedding": embedding, "base_mask_logits": base_logits,
                     "mask_logits": base_logits, "confidence": confidence, "detail": detail,
                     "memory_hw": memory_hw, "momd": None}
            if stage_index >= 2:
                stage["momd"] = mixture_decode(base_logits, detail["responsibility_class"], memory_hw,
                                                locality, materialize=run_pmec)
                if run_pmec:
                    stage["momd"]["reference_envelope"] = pca_reference_envelope(
                        stage["momd"]["base_probability"], confidence["joint"])
            stages.append(stage)

        mask_losses, pca_losses = [], []
        for stage_index, stage in enumerate(stages, 1):
            if stage_index == 1:
                mask_loss, detail = masked_query_bce(
                    stage["base_mask_logits"], targets, labels.bool(), stage["confidence"]["p_class"],
                    stage["confidence"]["joint"], locality)
            else:
                mask_loss, detail = mixture_class_bce(stage["momd"]["mixture"], targets, labels.bool())
        ...
        with torch.autocast(device_type=image.device.type, enabled=False):
            ...
            loss_pca = sum(weight * value for weight, value in zip(STAGE_WEIGHTS, pca_losses))
            loss_mask = sum(weight * value for weight, value in zip(STAGE_WEIGHTS, mask_losses))
        ...
        output = {
            ...
            "pixel_detail": pixel_detail, "query_detail": {"B": base, "semantic_feature": semantic_feature,
            "context_raw": context_raw, "context_feature": context_feature, "f4_memory": f4_memory},
            "radius": radius, "locality": locality, "primary_output": stages[-1]["momd"]["mixture"],
            "losses": {"loss": total, "loss_deep": loss_deep, "loss_pca": loss_pca, "loss_mask": loss_mask,
                       **{f"loss_pca_stage{i+1}": value for i, value in enumerate(pca_losses)},
                       **{f"loss_mask_stage{i+1}": value for i, value in enumerate(mask_losses)}},
        }
        if run_pmec:
            final = stages[-1]
            region, rows = pmec(final["base_mask_logits"].detach(), final["confidence"]["joint"].detach(),
                                final["confidence"]["p_class"].detach(), labels.bool(), locality)
            output["pmec_region"], output["pmec_rows"] = region, rows
            output["pmec_diagnostic_only"] = True
        return output
