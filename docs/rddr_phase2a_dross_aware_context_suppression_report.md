...
- Pretrained SHA256: `f668a2add80e33dfa8f1a0695df91f6d8cfad5ffbb26d1dc7bcd35903a1f6e16` (the same official `.params` file used by the locked training runs).
...
| Variant | Epoch | Classification loss | Exact match (%) | Training accuracy (%) | Backbone LR |
|---|---:|---:|---:|---:|---:|
| GS | 1 | 0.416726 | 39.8079 | 70.3916 | 0.00963958 |
| GS | 25 | 0.170203 | 72.2673 | 87.2800 | 9.553286e-7 |
| RCS | 1 | 0.416690 | 39.7780 | 70.3884 | 0.00963958 |
| RCS | 25 | 0.170401 | 72.2844 | 87.2874 | 9.553286e-7 |

两组均正常退出（exit_code=0），各 25 轮、29,275 步。完整 50 行训练记录、有限值和 LR 递减检查均通过；本轮不支持把指标下降归因为训练发散。由于没有进行逐 epoch segmentation evaluation，不能从这份记录判定 segmentation overfitting。
...
原 Phase-0 未长期保存逐像素 mask 文件，因此无法对比历史 mask 文件哈希。这里的可核验证据是：原代码、原 checkpoint、原进程后端设置重放，以及每张图五项 population count 完全匹配；新缓存另有 SHA256 manifest。详见 [evaluation provenance](rddr_phase2a_evaluation_provenance.md)。
...
Quintile Q5 uses the upper bin after quantile tie handling; frozen Top20 uses the historical `>=` threshold. They differ by five tied foreground pixels here and must not be presented as literally identical masks.
...
本次 RCS 的 Q1/Q5 mean(r) 分别为 0.992712/0.517564，context RMS ratio 为 0.993092/0.551365：门控确实执行了更强的高风险位置抑制，不是门控路径没有生效。但 Harmed-by-CH 净修复 +3.6038 pp 的同时，Corrected-by-CH 净变化为 −1.4420 pp；后者包含约 1,993 万像素，前者约 444 万。该结果支持“局部修复存在，但同时损伤原本能被校正的区域”，不支持整体 utility 已建立。
...
最终 gamma_context：C0=1.572197、GS=1.553347、RCS=1.532349；没有观察到 RCS 通过显著增大 gamma_context 抵消门控。RCS 的 EffectiveContextScale=1.255369，约为 C0 的 79.85%。GS/RCS mean(r) 接近（差约 0.068 个百分点），但 RMS ratio=0.832050/0.860608，并非能量严格匹配。

四类 IoU 均未超过当前同评估器 C0，下降主要集中在 Class 2（约 −0.3973 pp）和 Class 3（约 −0.4848 pp）。Small-region recall/mIoU 有改善，但不能抵消 CAM28_1 和 interior 的安全门槛失败；large-region mIoU 虽下降 −0.1820 pp，仍在其单项 −0.20 pp 容忍范围内。Gate C 失败具体来自前两项，不是所有安全子项均失败。
...
- Unit/integration tests: 15 passed, including none-mode equivalence, JSD/detach/range, zero added parameters, optimizer coverage, BF16 backward, fixed-quintile and official-metric checks.
- Full evaluator exit code: 0. No test or LUAD run was launched.
- CPU artifact verification: PASS. Recomputed overall metrics from all 3,418 per-image confusion matrices; verified all 10,000 bootstrap-value summaries and independently reconstructed the first 16 paired resamples.
- Every image's four historical CH counts and Top20 count match Phase-0. Gate A/B/C/D and the final decision were independently recomputed from the artifacts.
- Batch20 BF16 readiness peak allocation was 3,959,703,552 bytes for GS and 3,950,924,800 bytes for RCS. These are smoke measurements, not measured Full25 training peaks. The 3.155 GiB value above is evaluation peak memory.
- This reviewed report adds explanatory notes to the immutable machine-generated report. Original evaluation output, full CSV/JSON evidence and independent verification are in `audit/results/rddr_phase2a/`; server originals are in `/home/duyanhong/experiments/RDDR_PHASE2A_6f45ac7/report_final/`.
- The implementation-delivery workflow was used to add reproducible commands, provenance notes and artifact validation; no additional model or optimization change was made in this delivery.
